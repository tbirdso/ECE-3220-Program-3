...
int main(){
  unsigned int fd[20];
  char buffer1[32256], buffer2[1024], buffer3[1024];
  unsigned int length1, length2, count1, count2, count3;
  int i;

  const int line_size = 64;
  for(i = 0; i < MAX_FILE_SIZE; i += line_size) {
	sprintf( buffer1 + i, "%s",
  ...
  count1 = tfs_write( fd[0], buffer1, length1 / 4 );
  ...
  count1 = tfs_write(fd[10], buffer1, length1 / 2);
  printf("%d bytes written to f10\n", count1);
  count1 = tfs_write(fd[11], buffer1, length1 / 4);
  printf("%d bytes written to f11\n", count1);
  count1 = tfs_write(fd[12], buffer2, length2);
  printf("%d bytes written to f12\n", count1);
  count1 = tfs_write(fd[13], buffer1, length1 / 4);
  printf("%d bytes written to f13\n", count1);
  ...
  tfs_write(fd[16], buffer2, length2);
